 ...
 * components, the signing secret masked on display and stored as an
 * encryption envelope, and the receiver-side verification contract
 * written next to the controls it explains. Every write passes the
 * capability and nonce double gate and leaves an audit row.
 *
 * @package GreenPNG
 */
...
final class Gr_Webhooks_Page {
    ...
    private static function write_endpoint( int $id, string $action, Gr_Audit_Repository $audit, int $user_id ): string {
        ...
        if ( self::ACTION_ADD === $action ) {
            $error  = '';
            $new_id = Gr_Webhook_Repository::add( $url, $secret, $events, $active, $error );
            if ( 0 === $new_id ) {
        ...
        if ( Gr_Webhook_Repository::update( $id, $url, $secret, $events, $active ) ) {
            $audit->log(
                'update',
                'webhook_endpoint',
                (string) $id,
