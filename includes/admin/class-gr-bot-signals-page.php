...
final class Gr_Bot_Signals_Page {
    ...
    public static function render(): void {
        ...
        <div class="wrap">
            ...
            <?php endif; ?>
            <p class="description"><?php echo esc_html__( 'A segment match means the address sits inside an official Google, Bing, or Apple crawler segment whose engine the claimed agent also names — a confidence signal beside the DNS verdict, never a block, and never granted on the agent string alone.', 'greenpng' ); ?></p>
            <table class="widefat striped">
                <thead><tr>
                    ...
                    <th><?php echo esc_html__( 'Segments', 'greenpng' ); ?></th>
                    <th><?php echo esc_html__( 'Walks', 'greenpng' ); ?></th>
                    <th><?php echo esc_html__( 'Last seen', 'greenpng' ); ?></th>
                </tr></thead>
                <tbody>
                    <?php if ( array() === $verdicts ) : ?>
                        <tr><td colspan="7"><?php echo esc_html__( 'No crawler claims verified yet.', 'greenpng' ); ?></td></tr>
                    <?php else : ?>
                        <?php foreach ( $verdicts as $row ) : ?>
                            <?php
                            $signal = Gr_Crawler_Verify::segment_signal( (string) ( $row['ip'] ?? '' ), (string) ( $row['user_agent'] ?? '' ) );
                            ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    if ( '' === $signal ) {
                                        ...
                                        echo esc_html(
                                            sprintf(
                                                /* translators: %s: engine name, Google, Bing, or Apple. */
                                                __( '%s segment match', 'greenpng' ),
                                                ucfirst( $signal )
                                            )
                                        );
                                    }
