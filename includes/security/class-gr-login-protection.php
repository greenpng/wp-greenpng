...
final class Gr_Login_Protection {
    ...
    public static function register_hooks(): void {
        ...
        // The lockout event rides the bus without landing in the
        // marketing event stream: the security track already holds the
        // full-IP rows under lawful interest, and the behavior stream
        // stays anonymized (ADR-0005 dual-track).
        add_filter( 'gr_persist_event', array( self::class, 'skip_event_persistence' ), 10, 2 );
    }
    ...
    public static function record_failure( string $username, string $ip ): void {
        ...
        // The new-lock moment announces itself on the event bus (ADR-0019):
        // once per lock, never per attempt, so a notification subscriber
        // sees one clean transition instead of a storm of retries. The
        // payload carries the mask-worthy references; every render masks.
        gr_dispatch_event(
            'security_lockout',
            array(
                ...
                'username'    => $username,
                'ip'          => $ip,
                'reason'      => 'login lockout round ' . $round,
                'failures'    => $count,
                'threshold'   => $threshold,
                'round'       => $round,
                'duration'    => $ttl,
    ...
    /**
     * Keeps the security_lockout event out of the persisted event
     * stream: the bus still observes it live, but the marketing-side
     * rows never hold a raw address (the security log keeps those,
     * masked at display like every other render).
     *
     * @param bool                    $persist Default persistence vote.
     * @param \GreenPNG\Core\Gr_Event $event   The dispatched event.
     * @return bool
     */
    public static function skip_event_persistence( bool $persist, $event ): bool {
        return ( 'security_lockout' === $event->name() ) ? false : $persist;
    }
