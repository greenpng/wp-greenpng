...
final class Gr_Crawler_Verify {
    ...
    /**
     * The official-segment double proof (docs/07 §5.4): a source word
     * only when the address sits inside that engine's official
     * segment AND the claimed agent names the same engine. Either
     * half alone — a segment hit with a mismatched agent, an agent
     * string with no segment behind it — stays '', because trusting
     * a user-agent string alone is exactly the mistake the segment
     * subscription exists to prevent. The word is a confidence
     * signal that rides beside the DNS verdict; it never feeds a
     * block and never replaces the verdict.
     *
     * @param string $ip Client address as text.
     * @param string $ua Claimed user agent.
     * @return string 'google', 'bing', 'apple', or ''.
     */
    public static function segment_signal( string $ip, string $ua ): string {
        $hits = Gr_Spider_Segments::sources_for( $ip );

        foreach ( $hits as $source ) {
            $needle = 'google' === $source ? 'googlebot' : ( 'bing' === $source ? 'bingbot' : 'applebot' );

            if ( '' !== $ua && false !== stripos( $ua, $needle ) ) {
                return $source;
            }
        }
