...
abstract class Gr_Form_Adapter_Base implements Adapter_Interface {
    ...
    public function on_main( ...$args ): void {
        $this->guarded(
            function () use ( $args ): void {
                ...
                $this->note_challenge_pressure();
                $this->note_reputation_pressure();
    ...
    /**
     * Counts a form-bridge submission the security track already
     * convicted, for the progressive challenge's address counter
     * (ADR-0018 D5): a bot's successful ride through the form plugin is
     * exactly the pressure that should arm the challenge the address
     * meets on the login and registration pages. Runs in the two
     * funnel points only — never inside the consent gate — so bot
     * submissions count whether or not they carried a cookie.
     *
     * @return void
     */
    private function note_challenge_pressure(): void {
        Gr_Progressive_Verification::note_bridge_submission(
            Gr_Ip_Resolver::resolve(),
            (string) $this->identity->visitor_id()
        );
    }

    /**
     * Feeds the same funnel point to the reputation track: a
     * bot-convicted submission is the form-post failure the
     * AbuseIPDB counter counts (docs/12: only repeatedly failing
     * addresses are ever queried). Same evidence as the challenge
     * feed, independent arm condition.
     *
     * @return void
     */
    private function note_reputation_pressure(): void {
        Gr_Abuseipdb::note_form_submission(
            Gr_Ip_Resolver::resolve(),
            (string) $this->identity->visitor_id()
        );
    }
