...
final class Gr_Webhook_Repository {
    ...
    /** Plain JSON delivery: the v1.1 shape every existing endpoint keeps. */
    public const KIND_GENERIC = 'generic';

    /** Feishu/Lark bot: interactive card, sign + timestamp in the body. */
    public const KIND_FEISHU = 'feishu';

    /** WeCom group bot: markdown message. */
    public const KIND_WECOM = 'wecom';

    /** DingTalk custom robot: action card, timestamp + sign in the query. */
    public const KIND_DINGTALK = 'dingtalk';

    /** Slack incoming webhook: text + blocks. */
    public const KIND_SLACK = 'slack';

    /**
     * The closed kind vocabulary: every endpoint row stores one of
     * these, and rows written before the field existed read back as
     * the generic kind.
     */
    public const KINDS = array( self::KIND_GENERIC, self::KIND_FEISHU, self::KIND_WECOM, self::KIND_DINGTALK, self::KIND_SLACK );
    ...
    /**
     * The endpoint's delivery kind: a stored kind outside the
     * vocabulary — including the missing key on rows written before
     * kinds existed — reads back as generic, so no stored row ever
     * stops delivering.
     *
     * @param array<string, mixed> $endpoint Endpoint row.
     * @return string One of the KINDS words.
     */
    public static function kind_of( array $endpoint ): string {
        return self::valid_kind( (string) ( $endpoint['kind'] ?? '' ) );
    }
     ...
     * @param string             $kind   Delivery kind, one of the KINDS words.
     * @return int New endpoint id, or 0 on refusal.
     */
    public static function add( string $url, string $secret, array $events, bool $active, string &$error = '', string $kind = self::KIND_GENERIC ): int {
        $url = trim( $url );
        ...
        $rows[] = self::shape( $id, $url, $envelope, $events, $active, self::valid_kind( $kind ) );
     ...
     * after the same length gate, and an empty kind keeps the stored
     * one so an older caller cannot silently flatten a card kind.
     *
     * @param int                $id     Endpoint id.
     * @param string             $url    Delivery URL.
     ...
     * @param array<int, string> $events Event names.
     * @param bool               $active Whether deliveries run.
     * @param string             $kind   Delivery kind, '' to keep the stored kind.
     * @return bool
     */
    public static function update( int $id, string $url, string $secret, array $events, bool $active, string $kind = '' ): bool {
        $rows  = self::raw();
        ...
        $stored_kind = '' === $kind ? self::kind_of( $rows[ $index ] ) : self::valid_kind( $kind );

        $rows[ $index ] = self::shape( $id, $url, $envelope, $events, $active, $stored_kind, $rows[ $index ] );
    ...
    /**
     * The kind gate: one of the closed vocabulary or the generic
     * fallback, so an unknown word can never strand a row.
     *
     * @param string $kind Candidate kind.
     * @return string
     */
    private static function valid_kind( string $kind ): string {
        return in_array( $kind, self::KINDS, true ) ? $kind : self::KIND_GENERIC;
    }
     ...
     * @param string                    $kind    Delivery kind.
     * @param array<string, mixed>|null $previous Row being edited, null on add.
     * @return array<string, mixed>
     */
    private static function shape( int $id, string $url, string $secret, array $events, bool $active, string $kind = self::KIND_GENERIC, ?array $previous = null ): array {
        return array(
            'id'                   => $id,
            'url'                  => $url,
            'kind'                 => $kind,
