<?php
/**
 * Local country lookup (docs/07 §5.7, ADR-0007): zero outbound, zero
 * SQL, one L1 memo per request. Two data ladders share one answer:
 * a GeoLite2-Country database the owner downloaded (published under
 * uploads, read by the minimal MMDB reader) owns every lookup while
 * it is there; otherwise the bundled DB-IP data serves — fixed-width
 * binary records searched with fseek binary search, where an
 * override directory under uploads (written by the admin update arm)
 * wins over the bundled set, because the plugin directory is not
 * writable on WordPress.org hosting.
 *
 * @package GreenPNG
 */
...
final class Gr_Geoip {
    ...
    /** The GeoLite2 database file name inside that directory. */
    public const MMDB_FILE = 'GeoLite2-Country.mmdb';
    ...
    /**
     * Resolved GeoLite2 metadata, or false when no valid database
     * exists. Memoized per request.
     *
     * @var array<string, mixed>|false|null
     */
    private static $maxmind_meta = null;

    /**
     * The country for one address, '' when unknown or no data.
     *
     * The source ladder runs once per request: a valid GeoLite2
     * database the owner downloaded owns every lookup while it is
     * published (its '' is an honest unknown, not a fallthrough),
     * then the DB-IP override, then the bundled set.
     *
    ...
    public static function country( string $ip ): string {
        ...
        $code = '';
        if ( null !== self::maxmind_meta() ) {
            $code = Gr_Maxmind_Reader::lookup( self::maxmind_file(), $ip );
    ...
    public static function state(): array {
        $maxmind = self::maxmind_meta();
        if ( null !== $maxmind ) {
            // GeoLite2 reports no range counts; the build date is its
            // own version word, and it owns every lookup while it is
            // published.
            return array(
                'available'    => true,
                'source'       => 'maxmind',
                'build_date'   => gmdate( 'Y-m-d', (int) $maxmind['build_epoch'] ),
                'v4_ranges'    => 0,
                'v6_ranges'    => 0,
                'countries'    => 0,
                'generated_at' => gmdate( 'Y-m-d', (int) $maxmind['build_epoch'] ),
            );
        }
    ...
    /**
     * The published GeoLite2 file path inside the uploads directory.
     *
     * @return string
     */
    public static function maxmind_file(): string {
        return self::uploads_dir() . '/' . self::MMDB_FILE;
    }

    /**
     * Resolved GeoLite2 metadata, or null when the database is absent
     * or fails the reader's battery (the DB-IP ladder then serves).
     *
     * @return array<string, mixed>|null
     */
    private static function maxmind_meta(): ?array {
        if ( null !== self::$maxmind_meta ) {
            // False is the memo for "resolved, none or invalid".
            return false === self::$maxmind_meta ? null : self::$maxmind_meta;
        }

        $file = self::maxmind_file();
        $meta = is_readable( $file ) ? Gr_Maxmind_Reader::validate( $file ) : null;

        self::$maxmind_meta = null === $meta ? false : $meta;
     ...
     * a running request's feet. The GeoLite2 reader's handles drop
     * with them: an open handle keeps reading the renamed-away inode.
     *
     * @return void
     */
    public static function reset_for_tests(): void {
        ...
        self::$meta         = null;
        self::$memo         = array();
        self::$maxmind_meta = null;
        Gr_Maxmind_Reader::reset_for_tests();
    }
}
