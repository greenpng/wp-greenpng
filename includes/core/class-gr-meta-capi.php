...
final class Gr_Meta_Capi {
    ...
    public static function print_event_id( $order_id ): void {
        try {
            ...
            // Only meaningful when a server side that carries this id
            // is live. The derivation is shared: the Meta track and
            // the TikTok track both send this same event id, so either
            // service being configured makes the global worth
            // printing; exposing an id no server event will carry
            // helps nobody.
            if ( ! Gr_Http_Client::is_configured( Gr_Http_Client::SERVICE_META_CAPI )
                && ! Gr_Http_Client::is_configured( Gr_Http_Client::SERVICE_TIKTOK ) ) {
                return;
            }
