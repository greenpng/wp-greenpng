...
use GreenPNG\Integrations\Ecosystem\Gr_Param_Resolver;
...
if ( ! function_exists( 'gr_mask_email' ) ) {
    /**
     * Email display masking facade (ADR-0019, docs/21): masks username
     * for privacy compliance while preserving the domain part.
     *
     * @param string $email Raw email address.
     * @return string Masked email, or '' when invalid.
     */
    function gr_mask_email( string $email ): string {
        $email = trim( $email );
        ...
        if ( $len <= 1 ) {
            ...
            $masked_user = $user[0] . '***' . $user[ $len - 1 ];
        }

        return $masked_user . '@' . $host;
    }
}
...
if ( ! function_exists( 'gr_dynamic_event_register' ) ) {
    /**
     * Dynamic hook rule write facade (docs/03 §9). Refused shapes
     * return 0; rules only ever mount inside admin requests.
     *
     * @param array<string, mixed> $config hook, event (optional),
     *        map (optional: payload key => expression).
     * @return int New rule id, or 0 on refusal.
     */
    function gr_dynamic_event_register( array $config ): int {
        $map = isset( $config['map'] ) && is_array( $config['map'] ) ? $config['map'] : array();
        return Gr_Dynamic_Event_Repository::add(
            isset( $config['hook'] ) ? (string) $config['hook'] : '',
            isset( $config['event'] ) ? (string) $config['event'] : '',
            $map
        );
    }
}

if ( ! function_exists( 'gr_dynamic_events' ) ) {
    /**
     * Dynamic hook rule list facade (docs/03 §9).
     *
     * @return array<int, array<string, mixed>>
     */
    function gr_dynamic_events(): array {
        return Gr_Dynamic_Event_Repository::all();
    }
}

if ( ! function_exists( 'gr_dynamic_event_delete' ) ) {
    /**
     * Dynamic hook rule delete facade (docs/03 §9).
     *
     * @param int $id Rule id.
     * @return bool
     */
    function gr_dynamic_event_delete( int $id ): bool {
        return Gr_Dynamic_Event_Repository::delete( $id );
    }
}

if ( ! function_exists( 'gr_param_resolve' ) ) {
    /**
     * Param expression facade (docs/03 §9): args paths, auto: keys,
     * and literal passthrough.
     *
     * @param string               $expression Rule expression.
     * @param array<int, mixed>    $args       Hook arguments.
     * @param array<string, mixed> $context    Auto-key overrides.
     * @return mixed Resolved value, or null on a miss.
     */
    function gr_param_resolve( string $expression, array $args, array $context = array() ) {
        return Gr_Param_Resolver::resolve( $expression, $args, $context );
    }
}
