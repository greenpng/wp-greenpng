...
final class Gr_Contact_Repository {
    ...
    /**
     * Contact id for one cookie-track visitor, latest capture winning.
     * Notification-side resolution uses this when an event carries the
     * visitor reference instead of a contact id (the conversion card's
     * customer lookup): the bus never carries PII, so the card walks
     * back to the repository for its masked display fields.
     *
     * @param string $visitor_id Cookie-track visitor identity.
     * @return int Contact row id, 0 when the visitor has no contact.
     */
    public function id_for_visitor( string $visitor_id ): int {
        global $wpdb;
        ...
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- point lookup on the indexed visitor_id; latest capture wins by id order.
        return (int) $wpdb->get_var(
            $wpdb->prepare(
                // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- $table is a DDL-validated identifier from Gr_Database, not user input.
                "SELECT id FROM {$table} WHERE visitor_id = %s ORDER BY id DESC LIMIT 1",
                $visitor_id
            )
        );
    }
