...
Not by default, and never any GreenPNG-operated server. The optional integrations (GA4, Meta, TikTok, Matomo, Cloudflare Turnstile, hCaptcha, AbuseIPDB, and the built-in SMTP delivery to your own mail server) only send data after you enter credentials and enable them. The DB-IP geolocation database update, the optional MaxMind GeoLite2 download, and the search-spider segment lists only fetch when you click their update buttons (the last also offers an explicit weekly schedule you must enable). Crawler verification resolves connecting addresses through your server's normal DNS infrastructure and stores only the resulting verdict. Outbound webhooks only send to the https endpoints you registered yourself, and only for the event names you subscribed them to.
...
* **Outbound webhooks (your own endpoints)** — Purpose: notify systems you choose about conversion, lead, and behavior events. Sends: the event name and its parameters (identifiers travel in the plugin's own hashed forms, never an email address or IP) to the https URL you registered on the Webhooks page, signed with an HMAC-SHA256 header your receiver verifies. When: only for the event names you subscribed the endpoint to, and only for endpoints you added and activated; nothing is sent until you do. Failed deliveries retry twice (60s, then 300s) and stop visibly on the status page; the endpoint is not a third party we chose — it is yours. The payload format matches the endpoint's platform — generic JSON, Feishu interactive cards, WeCom markdown, DingTalk action cards (HMAC-SHA256-signed with the secret you set there), or Slack blocks — so each receiver gets its native shape; the delivery pipeline, retries, and this disclosure apply identically to every format.
* **Built-in SMTP delivery (your own mail server)** — Purpose: deliver this site's outgoing email through the SMTP server you configure, instead of the host's often-missing sendmail. Covers the plugin's own notification emails (order, lead, and security-lockout cards, whose bodies carry masked addresses and masked IPs only) and, once enabled, the whole site's native wp_mail stream (WooCommerce order mail, password resets, registration mail), which is composed by WordPress and WooCommerce themselves. Sends: that mail, plus your SMTP username and password/authorization code to the SMTP host you entered; the credential is stored AES-256-GCM-encrypted and never displayed back. When: only with the feature enabled and the host filled in; the "Send Test Email" button sends one test message to the address you enter in the same form.
* **TikTok Events API** — Purpose: server-side conversion tracking for TikTok ads. Sends: event name, order value and currency, and SHA-256-hashed identifiers only; no raw identifiers, no IP address, no user agent. When: on purchase and lead events, after marketing consent and only for sessions the bot filter already passed; the thank-you page exposes the shared event id so your existing browser pixel can deduplicate. Privacy policy: https://www.tiktok.com/legal/page/global/privacy-policy/en
* **Matomo Tracking (your own instance)** — Purpose: server-side conversion tracking against the Matomo you host yourself. Sends: the event name and its parameters (order value, currency), your site id and token, and a derived 16-hex visitor id (the plugin's own visitor binding, hashed again — Matomo's own cookie is never read), after marketing consent. When: on purchase and lead events like GA4. The endpoint is the instance URL you configure — the plugin sends nothing to any Matomo-operated service and works without one.
* **Cloudflare Turnstile** — Purpose: challenge login, registration, and form submissions that behave like automation. Sends: the one-time widget token from the visitor's browser and the visitor's IP address to Cloudflare's siteverify endpoint for validation; the token is single-use and never stored. When: the widget loads only after the same address repeatedly failed, the validation runs only on that submission, and both the site key and the secret key must be set with the feature enabled. Privacy policy: https://www.cloudflare.com/privacypolicy/
* **hCaptcha** — Purpose: challenge login, registration, and form submissions that behave like automation. Sends: the one-time widget token from the visitor's browser and the visitor's IP address to hCaptcha's siteverify endpoint for validation; the token is single-use and never stored. When: the widget loads only after the same address repeatedly failed, the validation runs only on that submission, and both the site key and the secret key must be set with the feature enabled. Privacy policy: https://www.hcaptcha.com/privacy
* **AbuseIPDB** — Purpose: reputation context for addresses that repeatedly fail logins and form posts. Sends: the connecting IP address, nothing else. When: one lookup per address, only with your API key and the feature enabled; results cache for 24 hours and the provider's daily quota stops the lookups on its own; the verdict is an advisory signal and never blocks by itself. Privacy policy: https://www.abuseipdb.com/privacy
* **Search-engine spider IP segments** — Purpose: keep search-crawler verification current. Sends: one request each to the search engines' published crawler-segment lists (Google's and Bing's JSON lists and Apple's published list), only when you click the refresh button on the IP Intelligence page or the optional weekly schedule you explicitly enable; no visitor data is sent. Receives: the official segment tables, merged into a local copy in your uploads directory.
* **MaxMind GeoLite2** — Purpose: optional override for the country database. Sends: your license key to maxmind.com only when you click the download button on the IP Intelligence page; no visitor data is sent. Receives: the GeoLite2 Country database into your uploads directory; the plugin never bundles it, and the GeoLite2 EULA forbids redistribution. EULA: https://www.maxmind.com/en/geolite/eula Privacy policy: https://www.maxmind.com/en/privacy-policy
...
= 1.2.2 =
* Modular email notifications and a built-in SMTP engine (ADR-0019): subscribe an inbox to order conversions, form leads, and security lockouts; every card renders masked addresses and masked IPs only, with customer display fields resolved from your own contact records rather than event payloads. The SMTP engine takes over the site's whole wp_mail stream through your own mail server — the password travels once from the settings form into AES-256-GCM storage, a one-click test button reports success or a human-readable diagnosis (expired authorization code, port/encryption mismatch, host firewall), and the lockout announcement fires once per lock at the threshold moment while staying out of the anonymized behavior event stream.
* E2E: the mini mock gained a recording SMTP sink and the notification feature a closed-loop pair of specs (settings arming, test button, a real Contact Form 7 lead, a real Store API order, a lockout fired at a test-network probe address), with WooCommerce's own order mail observed riding the same engine.

= 1.2.0 =
* Optional third-party integrations, every one off by default with its own credentials block, whole-or-nothing key storage, masked previews, and a local self-check: TikTok Events API and Matomo tracking join the existing GA4 and Meta conversion forwarding, all on the same consent-gated event surface with the shared event id exposed for browser-pixel deduplication.
* Progressive human verification: a Turnstile or hCaptcha challenge loads for an address only after it failed twice, only on the login and registration pages, and never site-wide; switching one provider on switches the other off, and each needs its own site key and secret key.
* IP Intelligence grew an AbuseIPDB reputation block (one lookup per repeatedly-failing address, 24-hour cache, provider-quota self-stop; the verdict is advisory and never blocks on its own), search-engine spider segments with reverse-DNS-aware confidence, and an optional MaxMind GeoLite2 override for country lookups — your license key, your click, downloaded into your uploads directory; the plugin never bundles it.
* Webhook endpoints can now post the receiver platform's own message-card shape — Feishu, WeCom, DingTalk, and Slack alongside the generic JSON document — with the same four signature headers and the same retry and circuit ladder on every type.
* A Plugin Ecosystem page: register third-party integration classes through one documented filter (probed for availability before mounting, errors surfaced on the status bus), and an opt-in dynamic event sniffer that maps any WordPress hook onto the event bus through admin-side rules with argument-path expressions and auto: context keys — the front end never mounts any of it.
