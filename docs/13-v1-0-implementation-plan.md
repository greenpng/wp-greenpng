...
| S4 | 设置服务 | ✅ | `includes/core/class-gr-settings.php`：唯一 autoload=yes 的 `gr_settings`（默认值集中定义，≤8KB），含安全/探针/归因/隐私默认 | 实测 2026-09-10：SettingsTest 6 tests（默认逐项对齐 ADR-0007、serialize 560 字节实测 <8KB、autoload=yes 写入、幂等不重置、默认合并、set 持久化）；:8091 `wp eval` 实测 install 后 `gr_settings` 数组就位（probe_default=1）、`gr_*` autoload=yes 恰 1 行（option list 实测）。"激活后 option 存在"字面闭环随 S6 激活钩子复测 | `05` §6 |
