...
- **前门执法（2026-09-13，ADR-0009 D3 落地，修复 C3 无消费方）**：显式封禁规则**无条件 403**（含 log 档——站长手写规则不适用档位模型的启发式谨慎，上表"仅高置信规则可升到此档"由此闭环，且允许列表永远优先于封禁臂）；临时锁**仅 block 档在前门执法**（log 档前台纯观察，登录梯度锁绝不泄漏为全站拦截——故静态臂与临时臂是分立谓词）；URL 允许规则**只豁免检测器、永不豁免封禁臂**（地址轴与内容轴正交）；被拒请求直写安全仓（`action_taken='blocked'`，W12 同款 403 形状）且**不经结论通道**（被拒地址永不喂 CRM 评分）；前门自身 Throwable 一律 fail-open（报 `gr_inspector_error`，页面照常、检测器照跑）。
