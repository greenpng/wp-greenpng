...
> 落地记录（2026-09-15，v1.1 Webhook 扇出，ADR-0016）：转化/线索事件的出网扇出计入**事件落地口径**——`gr_event` 总线上 `Gr_Webhook_Dispatcher` 每事件 1 次非 autoload option 读（`gr_webhooks`）+ 每匹配端点 1 次队列写入（AS 行或 WP-Cron 调度写）；线级 HTTP 一律在队列任务内（铁律 3），前台零同步外呼不变。无订阅端点时 = 1 次读、零写入。
