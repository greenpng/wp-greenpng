...
| 出网 Webhook 派发 | 无 `gr_webhook_dispatch()` 门面（设计期拟名，落地为队列管线）——`gr_event` 总线 → `Gr_Webhook_Dispatcher` 入队 → 队列任务 `Gr_Webhook_Delivery` 经 `Gr_Http_Client::post_raw` 出网（ADR-0016 D3） | 签名头 `X-Gr-Signature: sha256=<HMAC-SHA256(端点密钥, 原始 body)>`（发送体即签名体，字节稳定）；`X-Gr-Event`、`X-Gr-Delivery`（32hex 去重 id）、`X-Gr-Timestamp`（接收方 ±300s 防重放）；端点密钥站主自配、≥16 字符、信封存储、展示仅掩码 | v1.1（ADR-0016） |
...
> **U11 落地（2026-09-12）**：`gr_prune_table` → `GreenPNG\Storage\Gr_Retention::prune()`（date_col 标识符形状校验 `^[a-z_]+$` 拒注入面；days ≤0 保全部；批循环短批即停）。骑手 = `Gr_Retention::run_daily()` 挂 `gr_cron_daily_maintenance` 优先级 10（汇总 5 之后），**双轨**（settings `retention_days` + `retention_rows`）先到先执行、时间预算 10s 到点留明日；行数轨边界 = 第 max_rows+1 新行 id，DELETE `id <=` 边界（边界行自身即首个被逐行）。`optimize()` 手动 only——骑手三方法（run_daily/prune/prune_rows）源级反证无 OPTIMIZE（进四检），按钮经 `Gr_Data_Retention_Page`（SLUG greenpng-retention）。`gr_schedule_maintenance` 的调度本体已由 `Gr_Queue::boot()` 承担（W 系列落地）。
> **U12 落地（2026-09-12）**：`gr_get_table_stats` → `GreenPNG\Storage\Gr_Table_Stats::stats()`（information_schema 目录读+短键经 DDL 注册表反解滤外来表+transient 缓存 300s+flush；rows=引擎估算——容量读非账务读）。`gr_export_diagnostics` → `GreenPNG\Core\Gr_Diagnostics::export()`——**白名单字段集**：plugin/environment{php,wp,db,multisite,locale}/queue{backend,next_daily}/adapters/tables/generated_at；DB 家族以 `db_server_info()` 身份串为权威（CLI 下 `is_mariadb` 可为 NULL）；导出面 = `Gr_Status_Page`（SLUG greenpng-status，Tools 第四员）双门 POST+审计行+下载流（JSON 体+exit）。脱敏纪律的可测形：毒化凭据源后导出 JSON 零 secret/token/password/license 字样+IP 正则不中（进四检）。
