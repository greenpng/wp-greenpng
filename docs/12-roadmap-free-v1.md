...
> **修订记录**：2026-09-09 依据 iss/ 审计（结论见 `docs/14`）与站长决策（ADR-0007）：v1.0 改为**四支柱首发闭环**（探针 / A-B / CAPI / GeoIP 全进首发）；v1.1 聚焦运营增强；v1.2 生态开放。任务分解见 `docs/13`。2026-09-13 v1.1 启动：v1.1 各行的实现裁定见 ADR-0010~0016（冲销/IP 质量/行为/CRM/漏斗/弃购/Webhook），任务分解与进度真源移交 `docs/17`。
